...
describe("Baby JubJub js test", function (){
    this.timeout(200000);
    it("Compressing public keys should return same results as the 'pack_point' function from noir-elgamal", async () => {
        ...
        const publicKey = await bjj.privateToPublicKey(BigInt("0x0510bae26a9b59ebad67a4324c944b1910a778e8481d7f08ddba6bcd2b94b2c4"))
        const packedPublicKey = await bjj.packPoint(publicKey);
        ...
        const publicKey2 = await bjj.privateToPublicKey(BigInt("0x03d7c10d654c601b60f2380551b4782896335c48f5d56662b1d0604dd22c8568"))
        const packedPublicKey2 = await bjj.packPoint(publicKey2);
        ...
        const publicKey3 = await bjj.privateToPublicKey(BigInt("0x032336193e8d6ebf273828eb9d6600badbc7cd795c639dbe364307cd121473f2"))
        const packedPublicKey3 = await bjj.packPoint(publicKey3);
    ...
    it("Unpacking a packed public key should return the original public key", async () => {
        ...
        for (const private_key of private_keys) {
            const publicKey = await bjj.privateToPublicKey(BigInt("0x0510bae26a9b59ebad67a4324c944b1910a778e8481d7f08ddba6bcd2b94b2c4"));
            const packedPublicKey = await bjj.packPoint(publicKey);
            const unpackedPublicKey = await bjj.unpackPoint(packedPublicKey);
            assert.equal(unpackedPublicKey.x, publicKey.x);
            assert.equal(unpackedPublicKey.y, publicKey.y);
        }
    })

    it("Encrypting then decrypting should return the original value", async () => {
        ...
        const original_plaint_text = 109951162777;
        const { C1: encryptedValueC1, C2: encryptedValueC2 } = await bjj.elgamalEncryptPacked(publicKey, original_plaint_text);
        const decrypted = await bjj.elgamalDecrypt(privateKey, encryptedValueC1, encryptedValueC2, 8);
        assert.equal(original_plaint_text, decrypted);
    })

    it("Exponential ElGamal Encryption is indeed additively homomorphic", async () => {
        const generatedKeys = await bjj.generatePrivateAndPublicKey();
        const privateKey = generatedKeys.privateKey;
        const publicKey = await bjj.packPoint(generatedKeys.publicKey);
        const original_plaint_text1 = 1000;
        const original_plaint_text2 = 42;
        const { C1: encryptedValue1C1, C2: encryptedValue1C2 } = await bjj.elgamalEncryptPacked(publicKey, original_plaint_text1);
        const { C1: encryptedValue2C1, C2: encryptedValue2C2 } = await bjj.elgamalEncryptPacked(publicKey, original_plaint_text2);

        const encryptedValueSumC1 = await bjj.addPoints(encryptedValue1C1, encryptedValue2C1);// we sum the ciphertexts
        const encryptedValueSumC2 = await bjj.addPoints(encryptedValue1C2, encryptedValue2C2);// we sum the ciphertexts

        const decryptedSum = await bjj.elgamalDecrypt(privateKey, encryptedValueSumC1, encryptedValueSumC2, 8);
